...
class TestEGO(unittest.TestCase):
    def test_agent_name(self):  # Check if EGO Apollo is created
        ...
        self.assertEqual(agent.name, SimulatorSettings.ego_jaguar2015xe_apollo5)
    ...
    def test_different_spawns(self):  # Check if EGO is spawned in the spawn positions
        with SimConnection() as sim:
            ...
            agent2 = sim.add_agent(SimulatorSettings.ego_jaguar2015xe_apollo5, lgsvl.AgentType.EGO, state)
    ...
    def test_agent_velocity(self):  # Check EGO velocity
        with SimConnection(60) as sim:
            ...
            agent = sim.add_agent(SimulatorSettings.ego_jaguar2015xe_apollo5, lgsvl.AgentType.EGO, state)
    ...
    def test_equality(self):  # Check that agent == operation works
        with SimConnection() as sim:
            ego1 = sim.add_agent(SimulatorSettings.ego_jaguar2015xe_apollo5, lgsvl.AgentType.EGO, spawnState(sim))
            ego2 = sim.add_agent(SimulatorSettings.ego_jaguae2015xe_autowareai, lgsvl.AgentType.EGO, spawnState(sim))
    ...
    def create_EGO(self, sim):  # Only create an EGO is none are already spawned
        return sim.add_agent(SimulatorSettings.ego_jaguar2015xe_apollo5, lgsvl.AgentType.EGO, spawnState(sim))
